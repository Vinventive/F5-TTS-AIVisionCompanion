...
F5_TTS_PATH = os.getenv('F5_TTS_PATH')
F5_TTS_SAMPLE = os.getenv('F5_TTS_SAMPLE')
...
if F5_TTS_PATH:
    os.chdir(F5_TTS_PATH)
else:
    logging.error("F5_TTS_PATH not set in environment variables")
...
# TTS Variables and Model Loading
...
try:
    F5TTS_ema_model = load_f5tts_model(DiT, F5TTS_model_cfg, 1200000)
    logging.info("F5TTS model loaded successfully.")
except FileNotFoundError as e:
    logging.error(f"Failed to load F5TTS model: {e}")
    F5TTS_ema_model = None  # Handle as needed

def generate_audio(gen_text, ref_audio_path, ref_text, exp_name="F5-TTS"):
    if exp_name == "F5-TTS":
        ema_model = F5TTS_ema_model
    else:
        logging.error(f"Unknown TTS model: {exp_name}")
        return None, None
...
def generate_audio_sync(text, exp_name="F5-TTS"):
    # Reference audio and text
    ref_audio_path = F5_TTS_SAMPLE  # Update this path
    ref_text = F5_TTS_REF_TEXT   # Update this text
    generated_wave, sample_rate = generate_audio(text, ref_audio_path, ref_text, exp_name)
    ...
    # Convert to bytes
    buffer = io.BytesIO()
    sf.write(buffer, generated_wave, sample_rate, format='wav')
    buffer.seek(0)
